...
export async function createGoodsReceipt(data: {
  ...
  try {
    ...
    if (itemsToReceive.length === 0) {
      return { error: "At least one item must have a received quantity greater than 0" }
    }
    ...
    for (const item of data.items) {
      ...
      if (!poItem) {
        return { error: "Product not found in Purchase Order items" }
      }

      const alreadyReceived = receivedMap.get(item.productId) || 0
      if (alreadyReceived + item.quantityReceived > poItem.quantity) {
        return {
          error: `Received quantity exceeds ordered quantity for ${poItem.product.materialCode} (Ordered: ${poItem.quantity}, Already Received: ${alreadyReceived}, Attempting to Receive: ${item.quantityReceived})`
        }
      }
    }
